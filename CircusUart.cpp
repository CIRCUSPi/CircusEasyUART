      ...
      if (buffer[len] == '\r') {
        ...
        for (ii = 0; ii < this->handler_idx; ii++) {
          if (buffer[len] == this->handlers[ii].separator) {
            ...
            if (strncmp(this->handlers[ii].cmd, buffer,
                        strlen(this->handlers[ii].cmd)) == 0) {
              /* got it */
...
void CircusUart::loop() { this->receive(); }

uint16_t CircusUart::send(const char *cmd, const char separator,
                          const char *param) {
  uint16_t writeLen = 0;
...
size_t CircusUart::write(uint8_t data) { return this->stream.write(data); }

size_t CircusUart::write(const char *buffer, size_t size) {
  return this->stream.write(buffer, size);
}

void CircusUart::printHandlesInfo(void) {
  for (int i = 0; i < this->handler_idx; i++) {
    Serial.println();
    Serial.print("Index => ");
    Serial.println(i);
    Serial.print("Cmd => ");
    Serial.println(this->handlers[i].cmd);
    Serial.print("separator => ");
    Serial.println(this->handlers[i].separator);
    Serial.println();
  }
}
